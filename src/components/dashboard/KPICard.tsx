...
export function KPICard({ icon, label, value, subText, trend, tone }: KPICardProps) {
  ...
  return (
    <motion.div whileHover={reduceMotion ? undefined : { y: -2 }} transition={{ duration: 0.15 }} className="will-change-transform">
      <Card className={cn("relative overflow-hidden border-slate-300 bg-white shadow-sm transition-shadow hover:shadow-lg dark:border-slate-700 dark:bg-slate-900", tone.glow)}>
        ...
        <CardHeader className="pb-1">
          ...
          <CardTitle className={cn("text-[2rem] font-black leading-none tracking-tight", tone.text)}>{value}</CardTitle>
        </CardHeader>
        <CardContent className="pt-0">
          <p className="text-xs font-medium text-slate-500/80 dark:text-slate-400/80">{subText ?? "-"}</p>
        </CardContent>
      </Card>
    </motion.div>
  );
