        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">Manage multiple accounts, challenges, and brokers</p>
            </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex items-center gap-4">
            <div className="relative flex-1">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search accounts by name or broker..." className="w-full rounded-xl border-2 border-gray-200 bg-gray-50 py-3 pl-12 pr-4 focus:border-emerald-500 focus:outline-none" />
            </div>
            <div className="flex items-center gap-2">
              {(["ALL", "ACTIVE", "ARCHIVED"] as const).map((filter) => (
                <button key={filter} onClick={() => setFilterStatus(filter)} className={`rounded-lg px-4 py-2.5 text-sm font-medium transition-all ${filterStatus === filter ? "bg-emerald-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
...
function EmptyState({ onNewAccount }: { onNewAccount: () => void }) {
  return (
    <div className="py-20 text-center">
      <div className="mb-6 inline-flex rounded-full bg-gradient-to-br from-emerald-50 to-teal-50 p-6"><Wallet className="h-12 w-12 text-emerald-600" /></div>
      <h3 className="mb-2 text-xl font-bold text-gray-900">No trading accounts yet</h3>
      <p className="mx-auto mb-8 max-w-md text-gray-600">Create your first trading account to start tracking your performance across different brokers and challenges</p>
