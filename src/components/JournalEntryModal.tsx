...
export function JournalEntryModal({ open, editingId, form, onChange, onClose, onPickImage, onSave }: Props) {
  const moodOptions: Array<{ value: string; emoji: string; label: string; active: string; idle: string }> = [
    { value: "GREAT", emoji: "😊", label: "Great", active: "bg-emerald-500 text-white border-emerald-500", idle: "border-slate-300 hover:border-emerald-400" },
    { value: "GOOD", emoji: "🙂", label: "Good", active: "bg-teal-500 text-white border-teal-500", idle: "border-slate-300 hover:border-teal-400" },
    { value: "NEUTRAL", emoji: "😐", label: "Okay", active: "bg-slate-500 text-white border-slate-500", idle: "border-slate-300 hover:border-slate-400" },
    { value: "BAD", emoji: "😔", label: "Bad", active: "bg-rose-500 text-white border-rose-500", idle: "border-slate-300 hover:border-rose-400" },
  ];
        ...
        <div className="space-y-5 overflow-y-auto p-5">
              ...
              {moodOptions.map((mood) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs font-semibold">{mood.label}</p>
                  </button>
                );
              })}
            </div>
          ...
          {form.entryType === "TRADE" ? (
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div className="space-y-1"><Label className="flex items-center gap-1.5"><CheckCircle2 className="h-4 w-4 text-emerald-500" />What worked well?</Label><Textarea value={form.whatWentWell} onChange={(e) => onChange((p) => ({ ...p, whatWentWell: e.target.value }))} className="focus-visible:border-emerald-500 focus-visible:ring-emerald-500/25" /></div>
              <div className="space-y-1"><Label className="flex items-center gap-1.5"><AlertCircle className="h-4 w-4 text-amber-500" />What could be better?</Label><Textarea value={form.whatToImprove} onChange={(e) => onChange((p) => ({ ...p, whatToImprove: e.target.value }))} className="focus-visible:border-amber-500 focus-visible:ring-amber-500/25" /></div>
              <div className="space-y-1 md:col-span-2"><Label className="flex items-center gap-1.5"><Lightbulb className="h-4 w-4 text-purple-500" />Key Takeaway</Label><Input value={form.keyLesson} onChange={(e) => onChange((p) => ({ ...p, keyLesson: e.target.value }))} className="focus-visible:border-purple-500 focus-visible:ring-purple-500/25" /></div>
            </div>
          ) : (
